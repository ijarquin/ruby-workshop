...
test.describe("Header", () => {
  ...
  test("displays the three navigation links", async ({ page }) => {
    ...
    await expect(
      nav.getByRole("link", { name: "Saved Recipes" }),
    ).toBeVisible();
...
test.describe("Main content", () => {
  ...
  test("displays the page description", async ({ page }) => {
    await expect(
      page.getByText("Find and share the best recipes from around the world"),
    ).toBeVisible();
  });
});
...
test.describe("Ingredient search", () => {
  test("displays the ingredient input", async ({ page }) => {
    await expect(
      page.getByPlaceholder("Add an ingredient (e.g. Chicken)..."),
    ).toBeVisible();
  });
...
test.describe("Kitchen tips (initial state)", () => {
  test("displays 9 kitchen tip cards before any search is made", async ({
    page,
  }) => {
  ...
  test("displays recognisable tip titles", async ({ page }) => {
    await expect(
      page.getByRole("heading", { name: "Season in Layers" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Mise en Place" }),
    ).toBeVisible();
  });
});

test.describe("Recipe details panel", () => {
  test("adapts from 3-column to 1-column layout when switching to mobile", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 1280, height: 800 });
    await page.goto("/recipes");

    const ingredientInput = page.getByPlaceholder(
      "Add an ingredient (e.g. Chicken)...",
    );
    ...
    await page
      .getByRole("heading", { name: "Sugar Free Banana Bread" })
      .first()
      .click();
    await expect(
